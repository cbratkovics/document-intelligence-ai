...
class TestIntegrationRAG:
    ...
    async def test_full_rag_pipeline(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            files = {
                "file": (
                    "test.txt",
                    b"The capital of France is Paris. France is located in Western Europe.",
                    "text/plain",
                )
            }
            ...
            query_response = await client.post(
                "/api/v1/query", json={"text": "What is the capital of France?"}
            )
    ...
    async def test_streaming_response(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            query_data = {"text": "What is machine learning?", "stream": True}
    ...
    async def test_multiple_document_search(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            docs = [
                (
                    "doc1.txt",
                    b"Python is a programming language known for its simplicity.",
                    "text/plain",
                ),
                (
                    "doc2.txt",
                    b"JavaScript is widely used for web development.",
                    "text/plain",
                ),
                (
                    "doc3.txt",
                    b"Go is known for its concurrency features.",
                    "text/plain",
                ),
            ]
            ...
            search_response = await client.post(
                "/api/v1/search", json={"text": "programming languages", "top_k": 5}
            )
    ...
    async def test_document_deletion_and_search(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            files = {
                "file": (
                    "test.txt",
                    b"Unique content about quantum computing.",
                    "text/plain",
                )
            }
            upload_response = await client.post("/api/v1/documents/upload", files=files)
            ...
            search_response = await client.post(
                "/api/v1/search", json={"text": "quantum computing"}
            )
            assert search_response.status_code == 200
    ...
    async def test_concurrent_queries(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            files = {
                "file": (
                    "test.txt",
                    b"Test content for concurrent queries.",
                    "text/plain",
                )
            }
            ...
            tasks = [client.post("/api/v1/query", json=query) for query in queries]
    ...
    async def test_error_recovery(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            response = await client.post(
                "/api/v1/query", json={"text": ""}  # Empty query
            )
            assert response.status_code == 422  # Validation error

            # System should still work after error
            response = await client.post("/api/v1/query", json={"text": "Valid query"})
            assert response.status_code == 200
